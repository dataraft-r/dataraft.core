test_that("contract revisions retain identity and guarantee review checks", {
  ...
  expect_identical(dr_update_contract(old, version = "2"),
                   dr_contract_update(old, version = "2"))
  expect_error(dr_update_contract(old), class = "dataraft_error_contract")
  expect_error(dr_update_contract(old, old, version = "2"),
               class = "dataraft_error_contract")
  expect_error(dr_update_contract(old, version = "2", remove = "id"),
               class = "dataraft_error_contract")
})
...
test_that("contract slots work on products and workflow products without execution", {
  ...
  product <- dr_product("orders") |>
    ...
    dr_add_contract(old) |>
    dr_add_quality(function(data) stop("Rule must remain deferred"), name = "check")
  for (x in list(product, dr_workflow() |> dr_add_product(product))) {
    expect_identical(dr_extract_contract(x), old)
    expect_identical(dr_extract_contract(dr_update_contract(x, revised)), revised)
    expect_identical(dr_extract_contract(x), old)
    removed <- dr_remove_contract(x)
    expect_identical(dr_remove_contract(removed), removed)
    expect_error(dr_extract_contract(removed), class = "dataraft_error_definition")
    expect_error(dr_update_contract(removed, revised), class = "dataraft_error_definition")
    expect_identical(dr_extract_contract(dr_add_contract(removed, revised)), revised)
    edited_product <- if (inherits(removed, "dr_product_workflow")) removed$product else removed
  ...
  expect_error(dr_update_contract(product, revised, version = "3"),
               class = "rlib_error_dots_nonempty")
  expect_error(dr_remove_contract(dr_workflow()), class = "dataraft_error_definition")
})
...
test_that("named source edits are deferred, immutable and unambiguous", {
  ...
  for (empty in list(dr_product("orders"), dr_workflow())) {
    ...
    expect_error(dr_extract_source(multiple), class = "dataraft_error_definition")
    expect_error(dr_update_source(multiple, first), class = "dataraft_error_definition")
    expect_error(dr_remove_source(multiple), class = "dataraft_error_definition")
    expect_error(dr_update_source(x, second, name = "missing"),
                 class = "dataraft_error_definition")
    expect_identical(dr_extract_source(dr_remove_source(multiple, "other")), first)
    removed <- dr_remove_source(x)
    expect_identical(dr_remove_source(removed, "delivery"), removed)
    expect_error(dr_extract_source(removed), class = "dataraft_error_definition")
  }
})

test_that("workflow source edits do not silently reach into product sources", {
  product <- dr_product("orders") |> dr_add_source(data.frame(id = 1L), name = "delivery")
  flow <- dr_workflow() |> dr_add_product(product)
  expect_error(dr_update_source(flow, data.frame(id = 2L), "delivery"),
               class = "dataraft_error_definition")
  expect_identical(dr_remove_source(flow, "delivery"), flow)
  expect_identical(dr_extract_source(dr_extract_product(flow)), data.frame(id = 1L))
})

test_that("editing clears product preflight marks", {
  product <- dr_product("orders", data.frame(id = 1L)) |>
    dr_add_contract(c(id = "integer")) |> dr_validate()
  ...
  expect_null(attr(dr_update_source(product, data.frame(id = 2L)), "dr_validated"))
})
